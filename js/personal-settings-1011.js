(function () {
  ...
  function renderFolderRuleTable() {
    if (!folderRules.length) {
      return '<div class="sakuraalbum-empty">Noch keine Ordner-Ausnahmen. Der Standard gilt fuer alle Unterordner.</div>';
    }
  ...
  function renderFolderRuleRow(rule) {
    ...
    return `
			<tr data-folder-rule-id="${escapeAttr(rule.id)}">
				<td><input class="ska-rule-enabled" type="checkbox" ${rule.enabled === false ? "" : "checked"} title="Diese Ordnerregel verwenden."></td>
				<td><strong>${escapeText(rule.path)}</strong></td>
				<td>
					...
					<br><span class="sakuraalbum-field-help">${escapeText(folderRuleModeHelp(mode))}</span>
				</td>
				<td><input class="ska-rule-depth" type="number" min="0" max="${escapeAttr(adminSettings.maxScanDepth || 20)}" value="${escapeAttr(numberValue(rule.albumDepth, 1))}" ${customDepth ? "" : "disabled"} title="Eigene Tiefe ab diesem Ordner."></td>
				<td>
				...
				<td><input class="ska-rule-separator" type="text" maxlength="20" value="${escapeAttr(rule.separator || "")}" placeholder="${escapeAttr(settings.separator || " - ")}" title="Optionaler eigener Trenner fuer diese Regel."></td>
				<td><button class="ska-rule-remove sakuraalbum-button-danger" type="button" title="Ordnerregel entfernen.">Entfernen</button></td>
			</tr>
		`;
  }
