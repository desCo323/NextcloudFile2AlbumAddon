              ...
              .map(
                (album) => `
							<tr>
								...
								<td>${escapeText(album.readableFiles || 0)} / ${escapeText(album.fileCount || 0)}</td>
								<td>${escapeText(formatBytes(album.totalBytes || 0))}</td>
								<td>${escapeText(exportAlbumHint(album))}</td>
								<td><button class="ska-export-create" type="button" data-source-type="${escapeAttr(album.sourceType)}" data-source-id="${escapeAttr(album.sourceId)}" ${album.readableFiles > 0 ? "" : "disabled"}>Export starten</button></td>
							</tr>
						`,
              )
              .join("")}
