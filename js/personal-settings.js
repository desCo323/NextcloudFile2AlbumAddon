(function () {
  ...
  function render() {
    mount.innerHTML = `
			...
			<div class="sakuraalbum-toggle-row">
				...
				<label class="sakuraalbum-toggle">
						<input id="ska-videos" type="checkbox" title="Beruecksichtigt Videos, falls der Administrator das erlaubt." ${settings.includeVideos ? "checked" : ""}>
					Videos
				</label>
			</div>
			<div class="sakuraalbum-grid sakuraalbum-panel">
				...
				<div class="sakuraalbum-field">
					...
					<div class="sakuraalbum-field">
						...
						<input id="ska-depth" type="number" min="0" max="20" title="Ab dieser Tiefe werden tiefere Unterordner im naechsten Oberalbum gesammelt." value="${escapeAttr(numberValue(settings.albumDepth, 1))}">
					</div>
			</div>
    ...
    document
      .getElementById("ska-managed")
      .addEventListener("click", loadManagedAlbums);
    document
      .getElementById("ska-delete-preview")
      .addEventListener("click", deleteDryRunSelected);
    document
      .getElementById("ska-delete-all-preview")
      .addEventListener("click", deleteDryRunAll);
  ...
  async function save() {
    ...
    try {
      const response = await request(
        "/apps/sakuraalbum/api/v1/user/settings",
        "PUT",
        { settings: collect() },
      );
  ...
  async function preview() {
    ...
    try {
      const response = await request(
        "/apps/sakuraalbum/api/v1/preview",
        "POST",
        { settings: collect() },
      );
  ...
  async function dryRun() {
    ...
    try {
      const response = await request(
        "/apps/sakuraalbum/api/v1/sync/dry-run",
        "POST",
        { settings: collect() },
      );
      lastWriteRequest = response.canWrite
        ? {
            canWrite: true,
  ...
  async function writeAlbums() {
    ...
    if (
      !lastWriteRequest ||
      !lastWriteRequest.canWrite ||
      !lastWriteRequest.planFingerprint
    ) {
      status.textContent = "Bitte zuerst einen sicheren Dry-Run erstellen.";
      return;
    }
    const confirmation = window.prompt(
      "SakuraAlbum nutzt fuer das Erzeugen nur gespeicherte Einstellungen. Gib CREATE_ALBUMS ein, um den Schreibjob zu starten.",
    );
    ...
    try {
      const response = await request(
        "/apps/sakuraalbum/api/v1/sync/write",
        "POST",
        {
          confirmation,
  ...
  async function loadRuns() {
    ...
    try {
      const response = await request(
        "/apps/sakuraalbum/api/v1/sync/runs?limit=10",
        "GET",
      );
  ...
  async function loadManagedAlbums() {
    ...
    try {
      const response = await request(
        "/apps/sakuraalbum/api/v1/albums/managed?limit=200",
        "GET",
      );
  ...
  async function deleteDryRun(albumIds, deleteAll) {
    ...
    try {
      const response = await request(
        "/apps/sakuraalbum/api/v1/albums/delete/dry-run",
        "POST",
        {
          albumIds,
          deleteAll,
  ...
  async function deleteManagedAlbums() {
    ...
    if (
      !lastDeleteRequest ||
      !lastDeleteRequest.canDelete ||
      !lastDeleteRequest.planFingerprint
    ) {
      status.textContent =
        "Bitte zuerst eine sichere Loeschvorschau erstellen.";
      return;
    }
    ...
    try {
      const response = await request(
        "/apps/sakuraalbum/api/v1/albums/delete",
        "POST",
        {
  ...
  function resetPreparedPlans(event) {
    if (
      !event ||
      !event.target ||
      event.target.classList.contains("ska-managed-check")
    ) {
      return;
    }
    lastWriteRequest = null;
  ...
  function renderRuns(runs) {
    ...
    if (runs.length === 0) {
      output.innerHTML =
        '<div class="sakuraalbum-empty">Noch keine Laeufe vorhanden.</div>';
      return;
    }
