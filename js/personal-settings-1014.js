(function () {
  ...
  let folderRules = normalizeFolderRules(settings.folderRules, effectiveSettings.folderRules);
  let managedAlbums = [];
  let folderBrowser = null;
  let folderBrowserOpen = false;
  let folderBrowserTarget = "source";
  ...
  const STATUS_POLL_ACTIVE_MS = 7000;
  const STATUS_POLL_IDLE_MS = 30000;
  const STATUS_POLL_HIDDEN_MS = 60000;
  const LARGE_ALBUM_WARNING_MEDIA_COUNT = 1000;
  const VERY_LARGE_ALBUM_WARNING_MEDIA_COUNT = 5000;
  ...
  function render() {
    mount.innerHTML = `
			...
			<div class="sakuraalbum-toggle-row">
				...
				<label class="sakuraalbum-toggle">
					<input id="ska-auto-sync" type="checkbox" title="Bei Admin-Modus Bei Dateiaenderungen ist die Automatik automatisch aktiv, sobald SakuraAlbum fuer dein Konto eingeschaltet ist." ${autoSyncToggleChecked() ? "checked" : ""} disabled>
					Automatisch aktuell halten (Admin-Standard)
				</label>
			</div>
			...
			<div class="sakuraalbum-panel sakuraalbum-source-panel">
				<div class="sakuraalbum-panel-head">
					...
					<button id="ska-folder-open" type="button" title="Ordner aus deinem Dateienbereich als Quellordner auswaehlen.">Quellordner hinzufuegen</button>
				</div>
			...
			<div class="sakuraalbum-panel sakuraalbum-source-panel">
				<div class="sakuraalbum-panel-head">
					<div>
						<h3>Ordner-Regeln</h3>
						<p>Diese Ausnahmen gelten innerhalb deiner Quellordner. Damit kann ein Unterordner eine eigene Tiefe bekommen, als ein einzelnes Album gesammelt oder komplett ausgelassen werden.</p>
					</div>
					<button id="ska-folder-rule-open" type="button" title="Unterordner auswaehlen und eine Sonderregel anlegen.">Ordnerregel hinzufuegen</button>
				</div>
				${renderFolderRuleGuide()}
				${renderFolderRuleWarnings()}
				${renderFolderRuleTable()}
			</div>
			<div class="sakuraalbum-actions">
				...
				<button id="ska-export-center" type="button" title="Bereitet SakuraAlbum- oder Photos-Alben im Hintergrund als ZIP-Dateien vor.">Album-Downloads</button>
				...
				<button id="ska-diagnostic-csv" type="button" title="Laedt deine SakuraAlbum-Diagnose als CSV herunter und speichert eine serverseitige Kopie fuer spaetere Fehleranalyse.">Fehlerlog CSV</button>
			</div>
			...
			<div class="sakuraalbum-actions sakuraalbum-actions-danger">
				...
				<button id="ska-delete-stale-preview" type="button" title="Prueft nur veraltete SakuraAlbum-Alben, die nach den aktuellen Einstellungen nicht mehr geplant sind.">Veraltete pruefen</button>
  ...
  function effectiveStateText() {
    ...
    if (!settings.enabled) {
      return "Aktiviere SakuraAlbum fuer dein Konto und speichere. Danach wird die Erstgenerierung im Hintergrund vorgemerkt, wenn der Administrator Dateiaenderungen erlaubt.";
    }
    ...
    const active = allowed && settings.enabled;
    ...
    const nextAction = active
      ...
      : allowed
        ? "Aktiviere SakuraAlbum und speichere danach, dann startet die Erstgenerierung im Hintergrund."
  ...
  function renderFirstRunGuide() {
    const adminReady = adminSettings.enabled && effectiveSettings.adminGroupAllowed !== false;
    const appReady = adminReady && settings.enabled;
    const sourceReady =
      sourceFolders.some((source) => source.enabled !== false) ||
      (Array.isArray(adminSettings.defaultIncludePaths) && adminSettings.defaultIncludePaths.length > 0);
    const automationReady = appReady && adminSettings.autoSyncMode === "file_events";
    ...
    const steps = [
      {
        label: "1 App",
        title: "SakuraAlbum einschalten",
        text: appReady
          ? "Dein Konto ist aktiv. Neue Aenderungen duerfen vorgemerkt werden."
          : "Schalte SakuraAlbum fuer dein Konto ein und speichere.",
        done: appReady,
      },
      {
        label: "2 Quellen",
        title: "Quellordner waehlen",
        text: sourceReady
          ? `${activeSourceCount()} werden verarbeitet. Ueberschneidungen werden blockiert.`
          : "Waehle mindestens einen Ordner, zum Beispiel /Photos.",
        done: sourceReady,
      },
      {
        label: "3 Regeln",
        title: "Tiefe und Ausnahmen pruefen",
        text: folderRules.length
          ? `${folderRules.filter((rule) => rule.enabled !== false).length} Sonderregeln passen einzelne Ordner an.`
          : "Der Standard gilt fuer alle Unterordner. Ausnahmen kannst du pro Ordner setzen.",
        done: true,
      },
      {
        label: "4 Vorschau",
        title: "Plan ansehen",
        text: "Die Vorschau zeigt Alben, Medienzahl, Konflikte und Zusammenfassungen ohne Schreibaktion.",
        done: false,
      },
      {
        label: "5 Hintergrund",
        title: "Automatik laufen lassen",
        text: automationReady
          ? `${pending} wartend, ${processing} in Arbeit, ${failed} fehlerhaft. Cron verarbeitet die Queue ressourcenschonend.`
          : "Automatik startet erst, wenn Admin-Freigabe und Kontoaktivierung passen.",
        done: automationReady && pending === 0 && processing === 0 && failed === 0,
      },
    ];

    return `
			<div class="sakuraalbum-first-run">
				<div class="sakuraalbum-panel-head">
					<div>
						<h3>Visuelle Einrichtung</h3>
						<p>Diese Schritte fuehren dich von der ersten Auswahl bis zur automatisch gepflegten Albumstruktur. Die App schreibt erst im Hintergrund und nutzt dabei Admin-Limits.</p>
					</div>
				</div>
				<div class="sakuraalbum-steps">
					${steps.map((step) => renderFirstRunStep(step, firstPendingStepLabel(steps))).join("")}
				</div>
				<div class="sakuraalbum-actions sakuraalbum-quick-actions">
					<button id="ska-guide-source" type="button">Ordner waehlen</button>
					<button id="ska-guide-preview" type="button">Vorschau anzeigen</button>
					<button id="ska-guide-save" class="primary" type="button">Speichern und starten</button>
				</div>
			</div>
		`;
  }

  function renderFirstRunStep(step, activeLabel) {
    const stateClass = step.done ? "sakuraalbum-step-done" : step.label === activeLabel ? "sakuraalbum-step-active" : "";
    const stateText = step.done ? "Erledigt" : step.label === activeLabel ? "Naechster Schritt" : "Offen";
    return `
			<div class="sakuraalbum-step ${stateClass}">
				<div>
					<span>${escapeText(step.label)}</span>
					<strong>${escapeText(step.title)}</strong>
				</div>
				<p>${escapeText(step.text)}</p>
				<small>${escapeText(stateText)}</small>
			</div>
		`;
  }

  function firstPendingStepLabel(steps) {
    ...
    return `
			<div class="sakuraalbum-progress-panel">
				...
				<div class="sakuraalbum-progress-meta">
					...
					<span>${escapeText(queueNextRunText(queue))}</span>
  ...
  function renderSourceTable() {
    if (!sourceFolders.length) {
      return '<div class="sakuraalbum-empty"><strong>Noch keine Quellordner ausgewaehlt.</strong><span>Ohne Auswahl nutzt SakuraAlbum die Admin-Vorgabe. Fuer die normale Nutzung waehle einen Ordner wie /Photos aus und pruefe danach die Vorschau.</span></div>';
    }

    return `
			<div class="sakuraalbum-rule-card-list">
				${sourceFolders.map(renderSourceRow).join("")}
			</div>
		`;
  }

  function renderFolderRuleGuide() {
    return `
			<div class="sakuraalbum-rule-guide">
				<div>
					<strong>Eigene Tiefe</strong>
					<span>Ab diesem Unterordner gilt eine eigene Album-Tiefe. Bestehende SakuraAlbum-Alben werden beim naechsten Hintergrundlauf ergaenzt; alte verwaltete Alben werden nur entfernt, wenn der Admin die sichere Bereinigung aktiviert.</span>
				</div>
				<div>
					<strong>Alles in ein Album</strong>
					<span>Der Unterordner und alle Unterordner landen in einem einzigen Album.</span>
				</div>
				<div>
					<strong>Auslassen</strong>
					<span>Der Unterordner wird nicht fuer SakuraAlbum verarbeitet. Dateien bleiben unveraendert.</span>
				</div>
			</div>
		`;
  }

  function renderFolderRuleWarnings() {
    const warnings = folderRuleWarnings();
    if (!warnings.length) {
      return "";
    }

    return warnings.map((warning) => `<div class="sakuraalbum-warning">${escapeText(warning)}</div>`).join("");
  }

  function renderFolderRuleTable() {
    if (!folderRules.length) {
      return '<div class="sakuraalbum-empty"><strong>Noch keine Ordner-Ausnahmen.</strong><span>Der Standard gilt fuer alle Unterordner. Fuege nur dann eine Ausnahme hinzu, wenn ein bestimmter Ordner anders behandelt oder ausgelassen werden soll.</span></div>';
    }

    return `
			<div class="sakuraalbum-rule-card-list">
				${folderRules.map(renderFolderRuleRow).join("")}
			</div>
		`;
  }

  function renderFolderRuleRow(rule) {
    const mode = rule.mode || "depth";
    const customDepth = mode === "depth";
    return `
			<div class="sakuraalbum-rule-card" data-folder-rule-id="${escapeAttr(rule.id)}">
				<div class="sakuraalbum-rule-card-head">
					<label class="sakuraalbum-toggle">
						<input class="ska-rule-enabled" type="checkbox" ${rule.enabled === false ? "" : "checked"} title="Diese Ordnerregel verwenden.">
						Aktiv
					</label>
					<div class="sakuraalbum-rule-card-path">
						<strong>${escapeText(rule.path)}</strong>
						<span>${escapeText(folderRuleModeHelp(mode))}</span>
					</div>
					<button class="ska-rule-remove sakuraalbum-button-danger" type="button" title="Ordnerregel entfernen.">Entfernen</button>
				</div>
				<div class="sakuraalbum-rule-controls">
					<label>
						<span>Regel</span>
						<select class="ska-rule-mode" title="Sonderregel fuer diesen Unterordner.">
							...
							<option value="exclude" ${mode === "exclude" ? "selected" : ""}>Auslassen</option>
						</select>
					</label>
					<label>
						<span>Tiefe</span>
						<input class="ska-rule-depth" type="number" min="0" max="${escapeAttr(adminSettings.maxScanDepth || 20)}" value="${escapeAttr(numberValue(rule.albumDepth, 1))}" ${customDepth ? "" : "disabled"} title="Eigene Tiefe ab diesem Ordner.">
					</label>
					<label>
						<span>Name</span>
						<select class="ska-rule-template" title="Optionales eigenes Namensschema fuer diese Regel.">
							<option value="" ${(rule.namingTemplate || "") === "" ? "selected" : ""}>Standard</option>
							<option value="root_relative" ${rule.namingTemplate === "root_relative" ? "selected" : ""}>Hauptordner + Pfad</option>
							<option value="parent_leaf" ${rule.namingTemplate === "parent_leaf" ? "selected" : ""}>Oberordner + Ordner</option>
							<option value="leaf" ${rule.namingTemplate === "leaf" ? "selected" : ""}>Ordner</option>
						</select>
					</label>
					<label>
						<span>Trenner</span>
						<input class="ska-rule-separator" type="text" maxlength="20" value="${escapeAttr(rule.separator || "")}" placeholder="${escapeAttr(settings.separator || " - ")}" title="Optionaler eigener Trenner fuer diese Regel.">
					</label>
				</div>
			</div>
		`;
  ...
  function renderSourceRow(source) {
    ...
    const customDepth = mode === "depth";
    return `
			<div class="sakuraalbum-rule-card" data-source-id="${escapeAttr(source.id)}">
				<div class="sakuraalbum-rule-card-head">
					<label class="sakuraalbum-toggle">
						<input class="ska-source-enabled" type="checkbox" ${source.enabled === false ? "" : "checked"} title="Diesen Quellordner verwenden.">
						Aktiv
					</label>
					<div class="sakuraalbum-rule-card-path">
						<strong>${escapeText(source.path)}</strong>
						<span>${source.usesDefaultRules ? "Standardregeln. " : ""}${escapeText(sourceModeHelp(mode))}</span>
					</div>
					<button class="ska-source-remove sakuraalbum-button-danger" type="button" title="Quellordner entfernen.">Entfernen</button>
				</div>
				<div class="sakuraalbum-rule-controls">
					<label>
						<span>Regel</span>
					...
					<label>
						<span>Tiefe</span>
						<input class="ska-source-depth" type="number" min="0" max="${escapeAttr(adminSettings.maxScanDepth || 20)}" value="${escapeAttr(numberValue(source.albumDepth, 1))}" ${customDepth ? "" : "disabled"} title="Eigene Tiefe nur fuer diesen Ordner.">
					</label>
					<label>
						<span>Name</span>
					...
					<label>
						<span>Trenner</span>
						<input class="ska-source-separator" type="text" maxlength="20" value="${escapeAttr(source.separator || "")}" placeholder="${escapeAttr(settings.separator || " - ")}" title="Optionaler eigener Trenner fuer diesen Ordner.">
					</label>
				</div>
			</div>
		`;
  ...
  function bindMainActions() {
    ...
    document.getElementById("ska-export-center").addEventListener("click", () => loadExportCenter(false));
    document.getElementById("ska-diagnostic-report").addEventListener("click", loadDiagnosticReport);
    document.getElementById("ska-diagnostic-csv").addEventListener("click", downloadDiagnosticCsv);
    ...
    document.getElementById("ska-delete-stale-preview").addEventListener("click", deleteDryRunStale);
    ...
    const guideSource = document.getElementById("ska-guide-source");
    const guidePreview = document.getElementById("ska-guide-preview");
    const guideSave = document.getElementById("ska-guide-save");
    if (guideSource) {
      guideSource.addEventListener("click", () => document.getElementById("ska-folder-open").click());
    }
    if (guidePreview) {
      guidePreview.addEventListener("click", preview);
    }
    if (guideSave) {
    ...
    document.getElementById("ska-folder-open").addEventListener("click", () => {
      syncSourcesFromDom();
      syncFolderRulesFromDom();
      folderBrowserOpen = true;
      folderBrowserTarget = "source";
      render();
      loadFolders(folderBrowserPath);
    });
    document.getElementById("ska-folder-rule-open").addEventListener("click", () => {
      syncSourcesFromDom();
      syncFolderRulesFromDom();
      folderBrowserOpen = true;
      folderBrowserTarget = "rule";
      render();
      loadFolders(folderBrowserPath);
    });
  ...
  function bindFolderRuleActions() {
    Array.from(document.querySelectorAll("[data-folder-rule-id]")).forEach((row) => {
      const id = row.dataset.folderRuleId;
      row.querySelector(".ska-rule-enabled").addEventListener("change", (event) => {
        updateFolderRule(id, { enabled: event.target.checked });
        render();
      });
      row.querySelector(".ska-rule-mode").addEventListener("change", (event) => {
        updateFolderRule(id, { mode: event.target.value });
        render();
      });
      row.querySelector(".ska-rule-depth").addEventListener("input", (event) =>
        updateFolderRule(id, { albumDepth: Number.parseInt(event.target.value, 10) }),
      );
      row.querySelector(".ska-rule-template").addEventListener("change", (event) =>
        updateFolderRule(id, { namingTemplate: event.target.value }),
      );
      row.querySelector(".ska-rule-separator").addEventListener("input", (event) =>
        updateFolderRule(id, { separator: event.target.value }),
      );
      row.querySelector(".ska-rule-remove").addEventListener("click", () => {
        folderRules = folderRules.filter((rule) => rule.id !== id);
        lastWriteRequest = null;
        lastDeleteRequest = null;
        lastResetRequest = null;
        render();
  ...
  function bindFolderBrowserActions() {
    ...
    Array.from(document.querySelectorAll(".ska-folder-select, .ska-folder-select-current")).forEach((button) => {
      button.addEventListener("click", () => {
        if (folderBrowserTarget === "rule") {
          addFolderRule(button.dataset.path || "/");
          return;
        }
        addSourceFolder(button.dataset.path || "/");
      });
    });
  }
  ...
  function updateFolderRule(id, patch) {
    folderRules = folderRules.map((rule) =>
      rule.id === id ? normalizeFolderRule(Object.assign({}, rule, patch)) : rule,
    );
    lastWriteRequest = null;
    lastDeleteRequest = null;
    lastResetRequest = null;
  ...
  function syncFolderRulesFromDom() {
    Array.from(document.querySelectorAll("[data-folder-rule-id]")).forEach((row) => {
      updateFolderRule(row.dataset.folderRuleId, {
        enabled: row.querySelector(".ska-rule-enabled").checked,
        mode: row.querySelector(".ska-rule-mode").value,
        albumDepth: Number.parseInt(row.querySelector(".ska-rule-depth").value, 10),
        namingTemplate: row.querySelector(".ska-rule-template").value,
        separator: row.querySelector(".ska-rule-separator").value,
      });
    });
  }
  ...
  function collect() {
    syncSourcesFromDom();
    syncFolderRulesFromDom();
    const enabled = document.getElementById("ska-enabled").checked;
    ...
    return {
      ...
      autoSyncEnabled: adminSettings.autoSyncMode === "file_events" ? enabled : false,
    };
  }
  ...
  async function save() {
    ...
    try {
      ...
      folderRules = normalizeFolderRules(settings.folderRules, effectiveSettings.folderRules);
      lastWriteRequest = null;
      lastDeleteRequest = null;
      lastResetRequest = null;
      render();
  ...
  function downloadDiagnosticCsv() {
    const status = document.getElementById("ska-status");
    const url = `${OC.generateUrl("/apps/sakuraalbum/api/v1/diagnostics/logs.csv")}?limit=200`;
    status.textContent =
  ...
  async function loadStatus(silent) {
    if (statusRequestInFlight || document.visibilityState === "hidden") {
      return;
    }
  ...
  function addFolderRule(path) {
    syncSourcesFromDom();
    syncFolderRulesFromDom();
    const normalizedPath = displayPath(path);
    const exists = folderRules.some((rule) => rule.path.toLowerCase() === normalizedPath.toLowerCase());
    if (!exists) {
      folderRules.push(
        normalizeFolderRule({
          id: folderRuleId(normalizedPath),
          path: normalizedPath,
          enabled: true,
          mode: "depth",
          albumDepth: numberValue(settings.albumDepth, 1),
    ...
    render();
    document.getElementById("ska-status").textContent = exists
      ? "Fuer diesen Ordner gibt es bereits eine Regel."
      : "Ordnerregel hinzugefuegt. Bitte Vorschau pruefen und speichern.";
  }
  ...
  async function loadExportCenter(silent) {
    const status = document.getElementById("ska-status");
    const output = document.getElementById("ska-preview-output");
    exportCenterOpen = true;
    if (!silent) {
      status.textContent = "Lade Album-Downloads...";
      output.innerHTML = "";
    }
    try {
      const [albumsResponse, jobsResponse] = await Promise.all([
        request("/apps/sakuraalbum/api/v1/albums/export/albums?limit=200", "GET"),
        request("/apps/sakuraalbum/api/v1/albums/export/jobs?limit=20", "GET"),
      ]);
      exportAlbums = albumsResponse.albums || [];
      exportJobs = jobsResponse.jobs || [];
      renderExportCenter(albumsResponse, jobsResponse);
      updateExportPolling();
      if (!silent) {
        status.textContent = "Album-Downloads geladen.";
      }
    } catch (error) {
      if (!silent) {
        status.textContent = `Album-Downloads konnten nicht geladen werden: ${error.message}`;
      }
    }
  }

  async function createAlbumExport(sourceType, sourceId) {
    const status = document.getElementById("ska-status");
    status.textContent = "Merke Album-Export vor...";
    try {
      const response = await request("/apps/sakuraalbum/api/v1/albums/export", "POST", {
        sourceType,
        sourceId,
      });
      const job = response.job || {};
      status.textContent = `Export vorgemerkt: ${job.albumName || "Album"}. Der Hintergrundjob erstellt die ZIP-Dateien.`;
      await loadExportCenter(true);
    } catch (error) {
      status.textContent = `Export blockiert: ${error.message}`;
    }
  }

  function renderExportCenter(albumsResponse, jobsResponse) {
    const output = document.getElementById("ska-preview-output");
    const partSize = (albumsResponse && albumsResponse.partSizeBytes) || (jobsResponse && jobsResponse.partSizeBytes) || 1073741824;
    const activeJobs = exportJobs.filter((job) => ["pending", "running"].includes(job.status)).length;
    output.innerHTML = `
			<div class="sakuraalbum-summary">
				<div><strong>${escapeText(exportAlbums.length)}</strong><br>Alben</div>
				<div><strong>${escapeText(exportJobs.length)}</strong><br>Export-Jobs</div>
				<div><strong>${escapeText(activeJobs)}</strong><br>Aktiv</div>
				<div><strong>${escapeText(formatBytes(partSize))}</strong><br>Teilgroesse</div>
			</div>
			<div class="sakuraalbum-note">
				<strong>Downloads laufen im Hintergrund</strong>
				<span>Grosse Alben werden zuerst in deinem Dateienbereich unter SakuraAlbum Exports vorbereitet. Ab etwa ${escapeText(formatBytes(partSize))} entstehen mehrere Teil-ZIP-Dateien, damit der Webrequest nicht blockiert.</span>
			</div>
			${renderExportJobs()}
			${renderExportAlbums()}
		`;
    bindExportCenterActions();
  }

  function renderExportAlbums() {
    if (!exportAlbums.length) {
      return '<div class="sakuraalbum-empty">Keine Photos-Alben mit lesbaren Dateien gefunden.</div>';
    }

    return `
			<h3>Album exportieren</h3>
			<div class="sakuraalbum-table-wrap">
				<table class="sakuraalbum-preview-table">
					<thead>
						<tr>
              ...
              .map(
                (album) => `
							<tr>
								...
								<td>${album.managed ? "SakuraAlbum + Photos" : "Photos"}</td>
								<td>${escapeText(exportAlbumFileCountLabel(album))}</td>
								<td>${escapeText(exportAlbumSizeLabel(album))}</td>
								<td>${escapeText(exportAlbumHint(album))}</td>
								<td><button class="ska-export-create" type="button" data-source-type="${escapeAttr(album.sourceType)}" data-source-id="${escapeAttr(album.sourceId)}" ${albumExportButtonEnabled(album) ? "" : "disabled"}>Export starten</button></td>
							</tr>
						`,
              )
              .join("")}
  ...
  function renderExportJobs() {
    if (!exportJobs.length) {
      return '<div class="sakuraalbum-empty">Noch kein Album-Export vorbereitet.</div>';
    }

    return `
			<h3>Vorbereitete Downloads</h3>
			<div class="sakuraalbum-table-wrap">
				<table class="sakuraalbum-preview-table">
					<thead>
						<tr>
					...
					<tbody>
						${exportJobs.map(renderExportJobRow).join("")}
					</tbody>
				</table>
			</div>
		`;
  ...
  function renderExportJobRow(job) {
    const percent = Number.isFinite(Number(job.progressPercent)) ? Number(job.progressPercent) : 0;
    return `
			<tr>
				<td>${escapeText(job.albumName || "")}</td>
				<td>${escapeText(exportJobStatusText(job.status || ""))}${job.lastError ? `<br><span class="sakuraalbum-warning">${escapeText(job.lastError)}</span>` : ""}</td>
				<td>
					<div class="sakuraalbum-progress" role="progressbar" aria-valuemin="0" aria-valuemax="100" aria-valuenow="${escapeAttr(percent)}">
						<div class="sakuraalbum-progress-fill" style="width: ${escapeAttr(percent)}%"></div>
					</div>
					<span class="sakuraalbum-field-help">${escapeText(job.processedFiles || 0)} / ${escapeText(job.fileCount || 0)} Dateien, ${escapeText(formatBytes(job.processedBytes || 0))} / ${escapeText(formatBytes(job.totalBytes || 0))}</span>
				</td>
				<td>${escapeText(job.partCount || ((job.parts || []).length))}</td>
				<td>${escapeText(job.outputPath || "")}</td>
				<td>${renderExportPartButtons(job)}</td>
			</tr>
		`;
  }

  function renderExportPartButtons(job) {
    ...
    return parts
      .map((part) => {
        const path = "/apps/sakuraalbum/api/v1/albums/export/download";
        const href = `${OC.generateUrl(path)}?jobId=${encodeURIComponent(String(job.jobId))}&part=${encodeURIComponent(String(part.index || 1))}`;
        return `<a class="button" href="${escapeAttr(href)}" title="${escapeAttr(part.path || part.name || "")}">${escapeText(parts.length > 1 ? `Teil ${part.index}` : "ZIP")}</a>`;
      })
      .join(" ");
  }

  function bindExportCenterActions() {
    Array.from(document.querySelectorAll(".ska-export-create")).forEach((button) => {
      button.addEventListener("click", () =>
        createAlbumExport(button.dataset.sourceType || "photos", Number.parseInt(button.dataset.sourceId, 10)),
      );
    });
  }

  function updateExportPolling() {
    const hasActive = exportJobs.some((job) => ["pending", "running"].includes(job.status));
    if (!exportCenterOpen || !hasActive) {
      if (exportStatusInterval !== null) {
        window.clearInterval(exportStatusInterval);
    ...
    exportStatusInterval = window.setInterval(() => loadExportCenter(true), 5000);
  }

  function leaveExportCenter() {
  ...
  async function deleteDryRunStale() {
    const status = document.getElementById("ska-status");
    const output = document.getElementById("ska-preview-output");
    leaveExportCenter();
    status.textContent = "Pruefe veraltete verwaltete Alben...";
    output.innerHTML = "";
    lastDeleteRequest = null;
    lastResetRequest = null;
    try {
      const response = await request("/apps/sakuraalbum/api/v1/albums/delete/stale/dry-run", "POST", {});
      lastDeleteRequest =
        response.canDelete === true
          ? {
              albumIds: (response.albums || [])
                .map((album) => Number.parseInt(album.managedId, 10))
      ...
      status.textContent =
        response.canDelete === true
          ? "Veraltete Alben gefunden. Bitte Vorschau pruefen."
          : "Keine sicher loeschbaren veralteten Alben gefunden.";
      renderDeleteResult(response);
    } catch (error) {
      status.textContent = `Pruefung veralteter Alben blockiert: ${error.message}`;
    }
  }
  ...
  function renderManagedAlbums(response) {
    ...
    output.innerHTML = `
			...
			${renderLargeAlbumSummary(albums)}
			<div class="sakuraalbum-actions sakuraalbum-managed-actions">
				...
				<button id="ska-managed-stale-preview" type="button" title="Prueft nur verwaltete Alben, die im aktuellen Plan nicht mehr vorkommen.">Veraltete pruefen</button>
              ...
              .map(
                (album) => `
							<tr>
								...
								<td>${escapeText(album.mediaCount)}${renderAlbumLoadBadge(album.mediaCount)}</td>
    ...
    document.getElementById("ska-managed-stale-preview").addEventListener("click", deleteDryRunStale);
  ...
  function renderDeleteResult(result) {
    ...
    output.innerHTML = `
			...
			${summary.staleOnly ? renderStaleDeleteExplanation(summary, albums) : ""}
  ...
  function renderStaleDeleteExplanation(summary, albums) {
    if (!albums.length) {
      return `
			<div class="sakuraalbum-note">
				<strong>Keine veralteten SakuraAlbum-Alben gefunden</strong>
				<span>Die aktuell geplante Albumstruktur passt zu den verwalteten Alben. Es wird nichts geloescht.</span>
			</div>
		`;
    }
    ...
    return `
			<div class="sakuraalbum-note">
				<strong>Veraltete verwaltete Alben</strong>
				<span>Diese Alben gehoeren zu SakuraAlbum, kommen aber im aktuellen Plan nicht mehr vor. Das passiert zum Beispiel nach Ordnerumbenennungen oder geaenderten Regeln. Dateien bleiben erhalten; geloescht werden nur eindeutig verwaltete Photos-Alben oder verwaiste Tracking-Eintraege.</span>
				<span>Aktuell geplante Alben: ${escapeText(summary.currentPlannedAlbums || 0)}. Zu pruefende veraltete Alben: ${escapeText(albums.length)}.</span>
			</div>
		`;
  }
  ...
  function renderResetResult(result) {
    ...
    output.innerHTML = `
			<div class="sakuraalbum-summary">
				...
				<div><strong>${escapeText(summary.deletedDownloadJobs || 0)}</strong><br>Downloads</div>
				<div><strong>${escapeText(summary.removedDownloadQueueJobs || 0)}</strong><br>Export-Jobs</div>
  ...
  function renderDeleteTable(albums) {
    ...
    output.innerHTML = `
			...
			${renderPreviewTree(albums, (album) => (album.collision ? "Konflikt" : album.aggregated ? "Aggregiert" : "OK"))}
  ...
  function renderSyncResult(result) {
    ...
    output.innerHTML = `
			...
			${warnings.map((warning) => `<div class="sakuraalbum-warning">${escapeText(humanWarningText(warning))}</div>`).join("")}
			${renderPreviewTree(albums, (album) => actionLabel(album.writeAction, result.status))}
  ...
  function renderPreviewTree(albums, action) {
    if (!Array.isArray(albums) || !albums.length) {
      return '<div class="sakuraalbum-empty"><strong>Visuelle Vorschau</strong><span>Noch keine geplanten Alben sichtbar. Waehle einen Quellordner mit Medien oder pruefe Warnungen oben.</span></div>';
    }

    const groups = previewTreeGroups(albums);
    const totalShown = groups.reduce((count, group) => count + group.albums.length, 0);
    const hidden = Math.max(0, albums.length - totalShown);

    return `
			<div class="sakuraalbum-preview-tree">
				<div class="sakuraalbum-preview-tree-head">
					<div>
						<strong>Visuelle Vorschau</strong>
						<span>So wirken Quellordner, Tiefe und Ordner-Regeln auf die geplanten Alben.</span>
					</div>
					<span>${escapeText(totalShown)} von ${escapeText(albums.length)} Alben</span>
				</div>
				<div class="sakuraalbum-tree-list">
					${groups.map((group) => renderPreviewTreeGroup(group, action)).join("")}
				</div>
				${hidden > 0 ? `<div class="sakuraalbum-inline-help">${escapeText(hidden)} weitere Alben werden in der Tabelle angezeigt.</div>` : ""}
			</div>
		`;
  }

  function previewTreeGroups(albums) {
    const limit = 48;
    const groupMap = new Map();
    albums.slice(0, limit).forEach((album) => {
      const source = album.sourceRoot || firstPathSegment(album.targetPath || album.albumName || "/") || "/";
      if (!groupMap.has(source)) {
        groupMap.set(source, []);
      }
      groupMap.get(source).push(album);
    });

    return Array.from(groupMap.entries()).map(([source, groupedAlbums]) => ({
      source,
      albums: groupedAlbums.slice(0, 16),
    }));
  }

  function renderPreviewTreeGroup(group, action) {
    return `
			<div class="sakuraalbum-tree-group">
				<div class="sakuraalbum-tree-root">
					<span>Quelle</span>
					<strong>${escapeText(group.source)}</strong>
				</div>
				${group.albums.map((album) => renderPreviewTreeAlbum(album, action)).join("")}
			</div>
		`;
  }

  function renderPreviewTreeAlbum(album, action) {
    const status = action(album);
    return `
			<div class="sakuraalbum-tree-node">
				<div>
					<strong>${escapeText(album.albumName || "(ohne Namen)")}</strong>
					<span>${escapeText(album.targetPath || "")}</span>
				</div>
				<div class="sakuraalbum-tree-node-meta">
					<span>${escapeText(album.mediaCount || 0)} Medien</span>
  ...
  function normalizeFolderRules(storedRules, effectiveRules) {
    const base =
      Array.isArray(storedRules) && storedRules.length > 0
        ? storedRules
  ...
  function normalizeFolderRule(rule) {
    const path = displayPath(rule.path || "/");
    const mode = ["depth", "single_album", "exclude"].includes(rule.mode) ? rule.mode : "depth";
    const template = ["", "root_relative", "parent_leaf", "leaf"].includes(rule.namingTemplate || "")
      ? rule.namingTemplate || ""
      : "";
    return {
      id: /^[a-zA-Z0-9_-]{8,64}$/.test(rule.id || "") ? rule.id : folderRuleId(path),
      path,
      enabled: rule.enabled !== false,
      mode,
      albumDepth: clampNumber(rule.albumDepth, 0, numberValue(adminSettings.maxScanDepth, 20), numberValue(settings.albumDepth, 1)),
      namingTemplate: template,
      separator: String(rule.separator || "").slice(0, 20),
    };
  }
  ...
  function folderRuleWarnings() {
    const enabledSources = sourceFolders
      .filter((source) => source.enabled !== false)
      .map((source) => normalizeClientPath(source.path));
    return folderRules
      .filter((rule) => rule.enabled !== false)
      .filter((rule) => !enabledSources.some((source) => pathInside(normalizeClientPath(rule.path), source)))
      .map((rule) => `Ordnerregel ausserhalb der aktiven Quellordner: ${rule.path}. Sie wirkt erst, wenn ein passender Quellordner aktiv ist.`);
  }
  ...
  function startStatusPolling() {
    ...
    scheduleStatusPolling(pollIntervalMs());
    if (root.dataset.statusVisibilityBound !== "1") {
      ...
      root.dataset.statusVisibilityBound = "1";
    }
  }

  function scheduleStatusPolling(delayMs) {
    if (statusInterval !== null) {
      window.clearTimeout(statusInterval);
    }
    statusInterval = window.setTimeout(async () => {
      statusInterval = null;
      await loadStatus(true);
      scheduleStatusPolling(pollIntervalMs());
    }, delayMs);
  ...
  function pollIntervalMs() {
    ...
    const openWork =
      ...
      latest.status === "auto_chunk_partial";
    return openWork ? STATUS_POLL_ACTIVE_MS : STATUS_POLL_IDLE_MS;
  }

  function renderLargeAlbumSummary(albums) {
    const large = albums.filter((album) => Number(album.mediaCount || 0) >= LARGE_ALBUM_WARNING_MEDIA_COUNT);
    if (!large.length) {
      return "";
    }
    const biggest = large.reduce((max, album) =>
      Number(album.mediaCount || 0) > Number(max.mediaCount || 0) ? album : max,
    large[0]);
    return `<div class="sakuraalbum-warning">Performance-Hinweis: ${escapeText(large.length)} verwaltete Alben enthalten jeweils mindestens ${escapeText(LARGE_ALBUM_WARNING_MEDIA_COUNT)} Medien. Das groesste Album "${escapeText(biggest.albumName || "")}" hat ${escapeText(biggest.mediaCount || 0)} Medien; Nextcloud Photos muss beim Oeffnen sehr viele Vorschaubilder laden. Eine groessere Album-Tiefe oder Ordner-Regeln erzeugen kleinere, schneller ladende Alben.</div>`;
  }

  function renderAlbumLoadBadge(mediaCount) {
    const count = Number(mediaCount || 0);
    if (count >= VERY_LARGE_ALBUM_WARNING_MEDIA_COUNT) {
      return ' <span class="sakuraalbum-load-badge sakuraalbum-load-badge-heavy" title="Dieses Album ist sehr gross und kann in Nextcloud Photos lange laden.">sehr gross</span>';
    }
    if (count >= LARGE_ALBUM_WARNING_MEDIA_COUNT) {
      return ' <span class="sakuraalbum-load-badge" title="Dieses Album ist gross und kann in Nextcloud Photos spuerbar laenger laden.">gross</span>';
    }
    return "";
  }
  ...
  function autoStateLabel() {
    ...
    return settings.enabled ? "Aktiv" : "Bereit";
  }

  function autoSyncToggleChecked() {
    return adminSettings.autoSyncMode === "file_events" && (settings.enabled || settings.autoSyncEnabled || effectiveSettings.autoSyncActive);
  }
  ...
  function queueNextRunText(queue) {
    const pending = (queue && queue.counts && queue.counts.pending) || 0;
    const dueUsers = Number.parseInt((queue && queue.dueUsers) || 0, 10);
    const nextDueAt = queue && queue.nextDueAt ? Number.parseInt(queue.nextDueAt, 10) : 0;
    const job = (queue && queue.job) || {};
    if (dueUsers > 0) {
      return job.nextScheduledAt
        ? `Faellig, wartet auf Cron (${formatTime(job.nextScheduledAt)})`
        : "Faellig, wartet auf Cron";
    }
    if (nextDueAt > 0) {
      return `Naechster Lauf fruehestens ${formatTime(nextDueAt)}`;
    }
    if (pending > 0) {
      return "Vorgemerkt, wartet auf Entprellzeit";
    }
    return "Kein Lauf vorgemerkt";
  }
  ...
  function exportJobStatusText(status) {
    const labels = {
      pending: "Wartet auf Hintergrundjob",
      running: "ZIP wird vorbereitet",
      completed: "Bereit",
      failed: "Fehler",
    };
    return labels[status] || status || "";
  ...
  function exportAlbumHint(album) {
    if (album.exportBlockedReason === "max_export_files_exceeded") {
      return "Dieses Album ueberschreitet das Admin-Dateilimit fuer Hintergrund-Exports.";
    }
    if (album.exportBlockedReason === "max_export_bytes_exceeded") {
      return "Dieses Album ueberschreitet das Admin-Bytelimit fuer Hintergrund-Exports.";
    }
    if ((album.missingFiles || 0) > 0) {
      return `${album.missingFiles} Albumdateien sind nicht mehr lesbar und werden nicht exportiert.`;
    }
    if (album.totalBytesExact === false) {
      return "Groesseres Album: SakuraAlbum prueft die exakte Groesse beim Start des Hintergrund-Exports.";
    }
    if ((album.totalBytes || 0) >= 1073741824) {
      return "Grosses Album: SakuraAlbum erstellt Teil-ZIP-Dateien.";
    }
    return album.managed ? "Von SakuraAlbum verwaltet; Export nutzt das passende Photos-Album." : "Natives Photos-Album.";
  }

  function exportAlbumFileCountLabel(album) {
    const readable = album.readableFilesExact === false ? `>= ${album.readableFiles || 0}` : `${album.readableFiles || 0}`;
    return `${readable} / ${album.fileCount || 0}`;
  }

  function exportAlbumSizeLabel(album) {
    const size = formatBytes(album.totalBytes || 0);
    return album.totalBytesExact === false ? `mind. ${size}` : size;
  }

  function albumExportButtonEnabled(album) {
    return album.exportAllowed !== false && (album.fileCount || 0) > 0;
  }
  ...
  function folderRuleModeHelp(mode) {
    if (mode === "single_album") {
      return "Sammelt diesen Ordner und alle Unterordner in einem einzigen Album.";
    }
    if (mode === "exclude") {
      return "Laesst diesen Ordner komplett aus. Dateien werden nicht veraendert.";
    }
    return "Ab diesem Ordner gilt eine eigene Tiefe; tiefere Unterordner werden passend zusammengefasst.";
  }
  ...
  function humanWarningText(warning) {
    ...
    if (code === "folder_rule_skip") {
      return `Ordnerregel laesst diesen Bereich aus: ${warning.path || ""}`;
    }
    if (code === "folder_rule_outside_sources") {
      return `Ordnerregel liegt ausserhalb der aktiven Quellordner: ${warning.path || ""}`;
    }
    if (code === "invalid_folder_rule_path") {
      return `Ordnerregel hat einen ungueltigen Pfad: ${warning.path || ""}`;
    }
  ...
  function humanDeleteIssueText(issue) {
    ...
    if (code === "no_stale_managed_albums") {
      return "Es gibt keine veralteten SakuraAlbum-Alben fuer die aktuellen Einstellungen.";
    }
    if (code === "stale_cleanup_plan_truncated") {
      return "Die aktuelle Albumstruktur konnte wegen eines Limits nicht vollstaendig berechnet werden. Veraltete Alben werden deshalb nicht geloescht.";
    }
    if (code === "stale_cleanup_blocking_warning") {
      return `Die Pruefung veralteter Alben ist blockiert: ${issue.warningCode || "Warnung"}.`;
    }
  ...
  function folderRuleId(path) {
    let hash = 2166136261;
    String(path).split("").forEach((char) => {
      hash ^= char.charCodeAt(0);
      hash = Math.imul(hash, 16777619);
    });
    return `rule_${(hash >>> 0).toString(16)}_${String(path).length}`;
  }
