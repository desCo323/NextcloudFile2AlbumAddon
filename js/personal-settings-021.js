(function () {
  ...
  function renderLiveStatus() {
    ...
    const percent = progressPercent(latest, queue);
              ...
              .map(
                (sample) => `
							<tr>
								...
								<td>${escapeText(sample.status || "")}</td>
								<td>${escapeText(sample.changeCount || 0)}</td>
								<td>${escapeText(sample.lastError || sample.eventType || "")}</td>
							</tr>
						`,
              )
              .join("")}
  ...
  function renderCursorDetails(cursor) {
    return `
			<div class="sakuraalbum-inline-help">
				Hintergrund-Fortsetzung: Status ${escapeText(cursor.status || "")}, ${escapeText(cursor.processedFiles || 0)} verarbeitete Medienlinks, ${escapeText(cursor.processedAlbums || 0)} Album-Schritte, letzter Cursor ${escapeText(cursor.cursorPath || "Start")}.
			</div>
		`;
  }
  ...
  function progressPercent(run, queue) {
    const summary = (run && run.summary) || {};
    if (summary.progressPercent !== undefined) {
