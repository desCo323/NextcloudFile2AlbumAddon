  ...
  function renderAutoStatus(autoStatus) {
    ...
    output.innerHTML = `
			...
			<div class="sakuraalbum-note">
				...
				<span>${escapeText(autoStatus.maxUsersPerRun || 0)} Benutzer, ${escapeText(autoStatus.maxEventsPerRun || 0)} Events und ${escapeText(autoStatus.maxRuntimeSeconds || 0)} Sekunden pro Hintergrundlauf; Entprellzeit ${escapeText(autoStatus.debounceSeconds || 0)} Sekunden.</span>
			</div>
              ...
              .map(
                (item) => `
							<tr>
								...
								<td>${escapeText(item.changeCount || 0)} ${escapeText(item.eventType || "")}</td>
